...
FROM(
	...
	SELECT *,
	count(product_id) as purchase_count_per_qty
...
DROP TABLE if EXISTS cp_sales;
CREATE temp TABLE cp_sales as
...
FROM(
	...
	market_date
	,sales
	,row_number() OVER(ORDER by sales asc) as wrst_day
...
vendor_name
,product_name
,count(customer_id)*original_price as cost_per_product
...
VALUES(25,'Butter Tarts','10"',3,'unit',CURRENT_TIMESTAMP);
...
SET current_quantity=coalesce(
	...
	FROM(
			SELECT *
			,dense_rank() OVER(PARTITION by product_id ORDER by market_date DESC) rct_qty
			
			FROM vendor_inventory
	)x 
	WHERE rct_qty=1 and product_units.product_id=x.product_id),0);
